...
echo ""
echo "⚠ cdm must be run ONLY inside a cmux session."
echo ""
read -rp "Launch cmux now? [Y/n] " ans
if [[ ! "$ans" =~ ^[Nn]$ ]]; then
  echo "Starting cmux..."
  open -a cmux
  echo ""
  echo "Once inside cmux, run: cdm"
else
  echo ""
  echo "To get started later:"
  echo "  1. Launch cmux app:  open -a cmux"
  echo "  2. Inside cmux terminal:  cdm"
fi
