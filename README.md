...
Open the cmux app (or from terminal):

```bash
open -a cmux
```

### Step 3 — Run cdm inside cmux
...
> [!IMPORTANT]
> `cdm` must be run **only inside a cmux terminal**. If cmux is not running, start it first with `open -a cmux`.
...
- **Run `cdm` only inside cmux** — open the cmux app first, then run `cdm` in its terminal
